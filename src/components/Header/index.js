...
import { ContainerHeader, Logo, TituloSite, NavArea, Menu } from './style';
...
export default function Header(){
    ...
    return(
        <ContainerHeader theme={headerTheme} posScroll={posScroll}>
            ...
            <NavArea>
                {menu === false && <HiMenu size={25} color={headerTheme} onClick={() => {setMenu(true)}}/>}
                {menu &&
                ...
                {menu && mobile &&
                    (<AiOutlineClose size={20} color="#fff" onClick={() => {setMenu(false)}}/>)
                }
            </NavArea>
        </ContainerHeader>
    );
